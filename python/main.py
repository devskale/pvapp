...
def load_data():
    ...
    if not EXCEL_FILE_PATH.exists():
        ...
        print(f"Data file not found at {EXCEL_FILE_PATH}. Attempting to initialize environment...")
        try:
            # Assuming init_environment downloads to the correct relative path
            lf.init_environment() 
            if not EXCEL_FILE_PATH.exists():
                 raise HTTPException(status_code=500, detail=f"Data file {EXCEL_FILE_PATH} not found even after initialization.")
        except Exception as e:
             raise HTTPException(status_code=500, detail=f"Failed to initialize environment or find data file: {e}")
    ...
    try:
        ...
        if not isinstance(dfz, pd.DataFrame):
             if CATEGORIES_FILE_PATH.exists():
                 ...
                 print(f"Warning: Categories file {CATEGORIES_FILE_PATH} not found. Category names might be unavailable.")
                 dfz = None # Or handle as needed

        return df, dfz
    except ValueError as e:
        raise HTTPException(status_code=500, detail=f"Error loading dataframes: {e}")
    except FileNotFoundError:
         raise HTTPException(status_code=500, detail=f"Data file not found at {EXCEL_FILE_PATH}. Run initialization if needed.")
...
async def get_plot_day(
    ...
    kategorie: str = Query('H0', description="Energy category code (e.g., H0, G1)"),
    ...
    try:
        result = lf.plot_day(df, dfz, date, kategorie, yearly_sum, output='json')
...
async def get_plot_yeardays(
    ...
    try:
        result = lf.plot_yeardays(df, dfz, kategorie, year_str, yearly_sum, output='json')
        if result and 'error' in result:
             raise HTTPException(status_code=404, detail=result['error'])
        return result
    except ValueError as e:
...
async def get_categories():
    ...
    if dfz is None:
        raise HTTPException(status_code=404, detail="Category data (dfz) not loaded or available.")
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Internal server error: Missing expected column in category data: {e}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal server error retrieving categories: {e}")
